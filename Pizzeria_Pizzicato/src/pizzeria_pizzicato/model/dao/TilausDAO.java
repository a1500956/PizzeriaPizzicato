...
public class TilausDAO extends DataAccessObject {
	...
	public void addTilaus(Tilaus Tilaus, Ostoskori ostoskori) throws SQLException {
		...
		TuoteDAO tdao = new TuoteDAO(); //Luodaan 'TuoteDAO', jotta voimme hakea tuotteen tiedot. Ostoskorin objekteissa vain t‰ytteet & ID.

		try {

			connection = getConnection(); //Haetaan yhteys.

			//Valmistellaan 'Tilaus'-tauluun lis‰tt‰v‰t tilaukselle kuuluvat tiedot.
			...
			//Toteutetaan lis‰ys 'Tilaus'-tauluun.
			...
			//Emme tied‰ Tilaus-tauluun tehdyn tilauksen ID-lukua, mutta tarvitsemme sit‰, joten meid‰n tulee etsi‰ se.
			...
			for (int i = 0; i < ostoskori.getKoko(); i++) {
					...
					//'i'-luku toistuu yh‰ uudelleen useassa tilauksessa, mutta vain kerran jokaista tilausta kohden.
					//Sen tarkoitus on auttaa tunnistamaan tilauksen sis‰ll‰ juuri TIETTY tilauksen tuote, 
					//sill‰ useita saman ID-luvun omaavia tuotteita, joilla on jokin pieni eroavaisuus (esim. oregano) voi olla.
				...
				if(tdao.pizzaVaiJuoma(TX.getTuote().getId())){ //'pizzaVaiJuoma'-metodi ottaa selville tuotteen luonteen.
					p = (Pizza) TX.getTuote(); //Koska tuote ON pizza, voimme huoletta heitt‰‰ sen 'Pizza'-muotoon
					ArrayList<Tayte> lisataytteet = p.getTaytteet(); //Haetaan pizzan t‰ytteet.
					
					PizzaTayteDAO PTdao = new PizzaTayteDAO(); //Luomme PizzaTayte DAO:n, jolla otamme selv‰‰, mik‰li pizzassa on lis‰t‰ytteit‰.
					ArrayList<Tayte> alkupPizzanTaytteet = PTdao.haePizzanTaytteet(p.getId()); //Haemme pizzan aluper‰iset t‰ytteet.
					
					if(alkupPizzanTaytteet.size()<lisataytteet.size()){ //Jos pizzassa on enemm‰n t‰ytteit‰ kuin pit‰isi, on siin‰ lis‰t‰ytteit‰.
						
						//T‰m‰ metodi karsii kaiken paitsi tavallisuudesta poikkeavat lis‰t‰ytteet
						...
						for(Tayte t:lisataytteet){ //Lis‰‰mme j‰ljelle j‰‰neet t‰ytteet.
							stmtInsert = connection.prepareStatement(inserttaaLisataytteet);
							stmtInsert.setInt(1, tilauksenID);
							stmtInsert.setInt(2, i); //Lukua 'i' k‰ytet‰‰n aiemmin 'tilaus_rivi'-kohdan arvona, tilauksen sis‰isen‰ tuotteen yksilˆij‰n‰.
						...
						//T‰t‰ ei Teid‰n, yleisˆn, tarvitse huomioida, ellei jokin ole mennyt vikaan ja meid‰n tarvitsee paniikinsekaisesti korjata koodia edess‰nne.
					}
