...
public class listaaPizzat extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		for (int i = 0; i < pizzat.size(); i++) {
			...
			for (int j = 0; j < pTaytteet.size(); j++) {
				if (pizzat.get(i).getpId() == pTaytteet.get(j).getpId()) {
					pizzat.get(i).getTaytteet()
							.add(Integer.toString(pTaytteet.get(j).gettId()));
				}
			}
		...
		for (int i = 0; i < pizzat.size(); i++) {
			...
			listaaPizzat.get(i).setId(pizzat.get(i).getpId()); // PizzaMenulle
																// pizzanID

			for (int j = 0; j < pizzaLista.size(); j++) { // PizzaMenulle pizzan
															// nimi ID mukaan
